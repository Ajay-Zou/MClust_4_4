classdef MClustMainWindowClass < handle
    ...
    properties(Constant)
        AboutString = {[MClustSettings.VERSION], 'By AD Redish', ' ', ...
            ' including code by ', ' ', ...
            'F Battaglia; S Cowen', 'JC Jackson; P Lipa','NC Schmitzer-Torbert',  ...
            'SPC by B Hasz',...
    ...
    methods(Access=public)
        ...
        function RunSpcCutter(self)
            if isempty(MClust.GetData().FeatureTimestamps)
                warning('MCLUST:noDataLoaded', 'No data loaded.  Cannot start Cutter.');
            elseif isempty(self.SPCC) || ~isvalid(self.SPCC)
                self.SPCC = MClust.SpcCutter();
            else
                self.SPCC.GetFocus();
            end
        end
